$(document).ready(function() {
	...
	function initEventHandlers() {
		...
		$('#blastDuration').on('change', function() {
			blastDuration = $(document).data('plugin_sequencer').options.blastDuration = Number($(this).val());
			$('#blastDurationLabel').html(blastDuration);
		});
		
		// SCREEN PANEL
		$('#shakeCount').on('change', function() {
			shakeCount = $(document).data('plugin_sequencer').options.shakeCount = Number($(this).val());
	...
	function zoomInOut() {
		...
		zoomScalar = $(document).data('plugin_sequencer').options.zoomScalar;
		...
		$(document).sequencer('zoom', {element: 'characterWrapper', scalar: zoomScalar});
		
		//zoomEffect(zoomAmount, charZoomDuration);
		
	}
	
	function zoomEffect(zoomScalar, zoomDuration) {
		
		$(document).sequencer('zoom', {element: 'characterWrapper', scalar: zoomScalar, duration:zoomDuration})
	}
	...
	function moveIt() {
		$(document).sequencer('shakeScreen');
	}
	
	function blastIt() {
		$(document).sequencer('blast', {duration:1000});
	}
});